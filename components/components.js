...
export function SyncScreen(props){
  ...
  function seedAll(){
    ...
    setSyncConfig({url:url.trim(),apiKey:key.trim()});
    dbGetAll('bodycomp').then(function(bc){
      var all=props.strSessions.map(function(s){return queueSync('logStrength',s);}).concat(props.carSessions.map(function(s){return queueSync('logCardio',s);})).concat(bc.map(function(s){return queueSync('logBodyComp',s);}));
